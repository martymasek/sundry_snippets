...
# Transform each x,y coord and x,y sigma into 4 x,y coord pairs
plot_df <- example_df %>%
  mutate(c1_x = x_mu, # specify the calculation for each coordinate
         ...
         c3_y = y_mu - y_sigma,
         c4_y = y_mu) %>%
  select(-c(x_mu, y_mu, x_sigma, y_sigma)) %>% # get rid of these columns since we no longer need them for plotting
  pivot_longer(cols = !g,  # this says to pivot every column except the column named "g"
...
ggplot(plot_df) + 
  ...
  lims(x = c(0, NA),   # make the axes begin at 0 with an unspecified max
       y = c(0, NA)) +
